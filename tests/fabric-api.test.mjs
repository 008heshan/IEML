...
test('★ 理由里不许出现 markdown 星号（它会被原样显示出来）', () => {
  /*
   * 用户截图里 `**1.14**` 的星号原样显示在"当前组合不可用"面板里 ——
   * 那个面板与 `title` 提示都按**纯文本**渲染，不认 markdown。
   * 写进去就等于把排版符号端给用户看。
   */
  const r = fabricApiUnsupportedReason('1.7.10');
  assert.ok(!r.includes('**'), `理由里不该有 markdown 粗体，实际：${r}`);
  assert.ok(!/[*_`#]/.test(r), `理由里不该有任何 markdown 记号，实际：${r}`);
});
