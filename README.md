...
**当前版本：`0.1.0-beta.30`（公开测试版）** —— 版本号规则见
...
#   → src-tauri/target/release/bundle/nsis/IEML_0.1.0-beta.30_x64-setup.exe（安装程序）
...
| Quilt 安装（profile JSON） | ✅ **已实测**（2026-09-16 用户真机装好并启动成功）—— 与 Fabric 同一条 profile 合并路径 |
...
| 6 | ~~**Quilt 已实现但未实测**~~ | ✅ **已清**（2026-09-16 用户真机实测：能装、能启动） |
...
| 10 | ~~**没有做真机启动验证**~~ | ✅ **已清（2026-09-16 用户实测）**：用户多次真机启动（含"游戏关闭后仍显示运行"的复现、崩溃判据、以及**同时开两个实例**）。当时的担忧是"只截图没点启动"，现在有真实运行记录 |
| 11 | **资源中心没有"已装"标记** —— **按用户决定不做**（2026-09-15，用户原话"明确不做"） | ⚠️ `0.1.0-beta.2` 新增：列表里不会标出"这个项目你已经装了"—— 需要跨项目 id 与本地文件名/哈希比对，本轮没做。装重复了不会拦你 |
| 12 | ~~**多开实例没在真机上同时开过**~~ | ✅ **已清（2026-09-16 用户实测：同时开两个游戏正常）** —— 多开是 `0.1.0-beta.6` 做的（`running: HashMap<实例id, 进程>`，可单独停某一个），单元测试用的是真进程，但"两个 Minecraft 同时跑"一直只有测试证据 |
