...
/* 分形噪声（3 个倍频）：烟要有"大团的形 + 细碎的边"，单层噪声只会是一坨软斑 */
float fbm(vec2 p) {
  float v = 0.0;
  float a = 0.5;
  for (int i = 0; i < 3; i++) {
    v += a * vnoise(p);
    p *= 2.07;   // 非整数倍频：避免各层格点对齐出现方块感
    a *= 0.5;
  }
...
void main() {
  ...
  /* ★★ 烟雾缭绕：**三级域扭曲（fbm domain warping）** ——
   *
   *   把 uv 先用噪声推开，再照常算那三团光斑。于是**光斑的位置、颜色、大小
   *   一点没变**（用户要的"原来的那个光斑"），变的只是它们的边界：
   *   被揉成流动的烟絮，而不是干净的圆。
   *
   *   ★★ 2026-09-22（第五轮）：用户说「**烟雾缭绕的感觉不强**」——
   *      上一版只有两级**单层**噪声、幅度 0.13/0.045，揉出来只是"边缘有点毛"。
   *      这一版按 Inigo Quilez 那套 fbm warp 做三级：
   *        q = fbm(uv)        → 大团的形变（这一级决定"烟"的整体轮廓）
   *        r = fbm(uv + 4q)   → 在形变后的坐标上再取一次，得到卷曲的絮
   *        wuv = uv + 0.34(q-.5) + 0.16(r-.5)
   *      并且把噪声本身也换成 fbm（3 个倍频）—— 单层噪声只有一档尺度，
   *      无论幅度多大都只是"软斑"，出不来烟的层次。
   *
   *   ★ 配色与亮度**一律没动**（上一轮把它们推高过，用户的评价是"好丑"）：
   *     "更像烟"靠的是**结构**，不是更亮。
   */
  vec2 q = vec2(
    fbm(uv * 2.4 + vec2(0.0, uTime * 0.052)),
    fbm(uv * 2.4 + vec2(5.2, 1.3) - vec2(uTime * 0.045, 0.0))
  );
  vec2 r = vec2(
    fbm(uv * 4.3 + 3.6 * q + vec2(1.7, 9.2) + vec2(uTime * 0.033, 0.0)),
    fbm(uv * 4.3 + 3.6 * q + vec2(8.3, 2.8) - vec2(0.0, uTime * 0.028))
  );
  vec2 wuv = uv + 0.34 * (q - 0.5) + 0.16 * (r - 0.5);
  /* 烟还有"浓淡不均"：一团烟的边缘该有疏有密（用 smoothstep 拉开对比，
     不然只是整体变暗变亮，看不出"絮"） */
  float density = mix(0.55, 1.55, smoothstep(0.24, 0.86, fbm(wuv * 3.4 + vec2(uTime * 0.02, uTime * 0.014))));
