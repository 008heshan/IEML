    ...
    <>
      ...
      */}
      ...
      {/*
        立场提示条
        ★ 审计发现这里有两句**假话**，已改正：
          ① "IEML 只在启动时后台检查并打角标" —— 根本没有启动时自动检查，
             检查更新只有你点按钮那一条路。
          ② "旧文件会先移入回收站" —— `install_mod` 直接覆盖同名文件，
             文件名不同时旧 jar 会**留在原地**（于是同一个 Mod 两份，
             启动时可能直接崩）；`delete_mods` 也是直接删，没有回收站。
        界面上的每一句承诺都必须是代码真做到的。

        ★★ 2026-09-14 第十四轮：这里原来是**两大块 Note**（"Mod 不会自动更新"
           四行 + "装 Mod 的两条路"四行 + 一个目录按钮），合计压掉列表首屏的
           三分之一。现在压成一行：
             · 「打开 mods 目录」升到右上角工具条（它是个**动作**，不该埋在说明里）；
             · "两条路"里"点添加 Mod 搜索安装"这件事，按钮本身就在旁边，不用讲；
             · "不会自动更新 / 原地覆盖"这条**必须留**（它是真话，而且会咬人），
               一行写完。
      */}
      <div className="mp-note">
        <IconShield />
        <span>
          IEML <b>不会自动更新 Mod</b> —— 更新要你点右上角「检查更新」手动做；
          装的时候<b>直接覆盖同名文件</b>，文件名变了会在 mods 里多出一个 jar，装完回来核对一下。
        </span>
      </div>
