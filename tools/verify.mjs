...
/*
 * ★★ **版本清单按世代分组**（2026-09-26，用户截图：「这个快照的显示非常混乱」）。
 *
 *   为什么是门禁：这类错**不报错、只是显示乱** ——
 *   同一个世代被切成两段（上游清单里 `26.2` 与 `26.2-rc-2` 不相邻），
 *   用户看到"快照出现两次、26.1 出现两次"，而没有任何一处会红。
 *   ★ 这条判据能**真的调那两个函数**：它们为此从 `.tsx` 搬到了 `version-family.ts`
 *     （`.tsx` 跑不进 `node --test`，见 `tests/version-groups.test.mjs` 文件头）。
 */
results.push([
  '版本世代分组',
  run('版本世代分组', 'node', ['--test', 'tests/version-groups.test.mjs']),
]);
/*
