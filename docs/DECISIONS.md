...
> ★★ **2026-09-25 修标题**：这条的**标题**一直写着"三段式流水线"，而**正文在第六轮就被重写成了**
> "日志特征匹配为主（9 大类约 70 条正则）+ 堆栈启发式为辅"。标题与正文相反，
> 谁只看标题就会照着**已经废弃的**架构去写。以正文的"第六轮重写"那一节为准。
> （★ 编号与锚点都不动 —— 别的地方还在引用 `#adr-011…`。）
...
> ★★ **2026-09-25 加注（这条与事实冲突了，但**不改写原决定**）**：
> 上面"多套装饰主题不做、深浅两套足够"这句**已经不成立** —— 事实是
> ① 2026-09-16 用户要求**删掉白色模式**（浅色那一套没了）；
> ② 2026-09-22 用户要求**九套主题**，落在 `src/styles/tokens.css` 的九个 `:root[data-theme]` 块里。
> 所以现状是"**只有深色底 + 九套配色**"，与这条的两种说法都不一样。
> ★ 为什么不直接改这张表：它记的是**当时的取舍**（存档价值）。
> 要看现状请读 `tokens.css` 与 ADR-059 的第二十节；**别拿这张表去决定"该有几套主题"**。
> ★ 另一条仍然有效并值得保留：**每一个新功能都要先过一遍"它值多少 KB"**。
...
[ADR-028](#adr-028实例设置与全局设置必须视觉区分-第九轮新增)（同一套"作用域必须说清"的思路）、
...
[ADR-020](#adr-020加载器识别必须用libraries-坐标--排除条件-第六轮新增)（加载器识别改走库坐标；
★ 2026-09-25 修：这里原来指的是一个**不存在的锚点** `#adr-020主页即当前实例概览`，
而 ADR-020 从头到尾都是"加载器识别"那一条 —— 死链接比没链接更费时间）、
[ADR-028](#adr-028实例设置与全局设置必须视觉区分-第九轮新增)（作用域的视觉区分，本决策沿用）、
[ADR-033](#adr-033繁复来自留白与层级而非功能数量密度收敛与三条硬约束)（同一轮配套的减法）、
...
4. 清单请求加**总超时**（默认 75 秒，可用 `IEML_LOADER_TIMEOUT_SECS` 调）：
   ...
   > ★★ **2026-09-25 加注：这一条已被 ADR-049 取代。** ADR-049（按 PCL2 的路子做网络优化）
   > 把"五个来源共用 75 秒"改成了**每个来源自己的 20 秒** —— 理由是共用总超时会让
   > "一个慢来源拖住其余四个"。现行实现看 `net/metadata.rs`（`IEML_LOADER_TIMEOUT_SECS` 仍在，
   > 但语义是"每个来源"）。★ 这里不删原文：它记的是当时的实测依据（30 秒会误杀 Fabric）。
...
**日期**：2026-09-16
**状态**：已实施
**修正**：[ADR-026](#adr-026多源竞速与镜像表-第六轮新增) 的镜像表顺序、
...
> ★★ **2026-09-26（第七十八轮）：两仓合并了，上面的"分开"作废。**
> 用户要求「合并代码仓与发布仓，CNB 的代码仓直接作为分发仓」。
> ⇒ 端点改成 **`https://cnb.cool/IEML_Official/IEML/-/releases/download/latest/latest.json`**，
> `publish-cnb.mjs` 的 `REPO` 常量也随之改成代码仓。
>
> ★ 但"必须公开"这条**没变、也不可能变** —— 再次实测：
> · 代码仓 release 的匿名下载 → **404**（当时代码仓是私密的）；
> · 旧发布仓的 `latest.json` 匿名下载 → **200** ✓。
> CNB 官方文档《角色权限》也写明：Release 附件的「下载」对 **`匿名`** 只在**公开仓**放行。
> ⇒ **合并的前提是代码仓转公开**（这次和 GitHub 一起转）。
>
> ★★ **桥接（顺序错了就断更新）**：`latest.json` 的地址**编在 exe 里**，
> 所以已经装了 rc.8 及更早版本的客户端只会去问**旧发布仓**。因此 rc.9 这一版
> **必须同时发到旧发布仓**（`publish-cnb.mjs --upload --bridge`），
> 老客户端更新到 rc.9 之后才开始读代码仓；此后 `--bridge` 就可以不用了。
> **发布顺序：先 --bridge 发旧的，再发新的。**
>
> ★★ **2026-09-26（同一天，第二段）：旧发布仓**才**算真收尾了** —— 用户看完第一段之后指出
> 「cnb 的两个库没有合并（发布库没有废除）」，核对属实：那时旧仓里还躺着 **10 个 release**
> （rc.1~rc.9 + beta.56/.57）与 **22 个 tag**，等于**两套发布面并存**。
> 处置（每一步都实测过，不是推断）：
>
> | 步 | 做了什么 | 实测 |
> |---|---|---|
> | 1 | 旧仓 README 改成弃用告示，列出"这里剩下什么 / 什么时候能删" | 推上去（本地 clone 后 push，没在 `E:\IEML` 里动文件） |
> | 2 | 仓库简介改成「【已停用 · 仅为老客户端保留】…」 | `PATCH /{repo}` |
> | 3 | 删掉 10 个重复 release 与 21 个失效 tag，只留 `latest` + `v0.1.0-rc.9` | 列表每次只回 10 条 ⇒ **要循环删**（见下面那条教训） |
> | 4 | ★★ **把桥做成"自足"**：`latest.json` 的包地址从**代码仓**改成**旧仓自己的副本** | 这一步是核对第 3 步时发现的：桥清单原本指向代码仓那个包 ⇒ 代码仓的包一坏，**老客户端连退路都没有** |
> | 5 | 把 rc.9 的包重新放回旧仓 `v0.1.0-rc.9`（第 3 步删掉了它） | 匿名下载 3.41 MB、MZ 头 |
> | 6 | 归档旧仓（`POST /{slug}/-/settings/archive`，`status` 0 → 1） | **归档后匿名下载实测仍通** —— 这一步不能想当然，不通就得当场解归档 |
>
> ★ 留下了 **`tools/probe/probe-legacy-endpoint-bridge.mjs`**：7 条判据（清单可读 / 版本 /
> 包地址是自足的 / 包可下且是 PE / 退路可下）—— 桥断掉是**静默**的（老客户端只显示
> "检查更新失败"），所以它必须能一条命令重跑。
>
> ★★ **`publish-cnb.mjs` 加了"桥冻结"闸门**：桥里那份清单是**手改过的形态**（自足），
> 而脚本生成的那份指向代码仓 ⇒ 拿脚本再刷一次桥 = 把自足改回**单点**。
> 现在同版或更新版本再跑 `--bridge` 会**直接报错**，只有显式
> `--bridge-republish` 才放行（附 `compareVersions`，判 `0.1.0-rc.9 < 0.1.0` 这类跨阶段比较）。
>
> ★ 教训（两条都值得留）：
> · **"列表接口给了我几条"不等于"仓里有几条"**：旧仓有 11 个 release，列表只回 10 条，
>   而 `latest` 恰好不在里面 —— 我第一版脚本差点把桥一起删掉。改法：**桥按 id 排除**，
>   并且**循环取列表直到没有可删的**。
> · **删掉自己的退路时不自知**：第 3 步删 rc.9 之后，桥清单指向**代码仓**那个包，
>   于是"桥的退路"其实一直是零。**删重复产物之前，先看清现存链路依赖的是哪一份。**
...
### 五十七、改向：安装游戏**并回下载页**（0.1.0-rc.1，2026-09-23 晚）

#### 57.1　用户的新指令，以及它**没有**推翻什么

用户原话：**「首先，把安装版本合并到下载里；1.转到下载页；2.写「无」」**。

同一天早些时候（ADR-056）用户要的是"单开一页"。**两条不矛盾**，因为它们治的不是同一件事：

| | 治什么 | 结果 |
|---|---|---|
| ADR-056 | 一屏里"两层导航 + 十几个控件" | **两步向导**（留下） |
| 本条 | 侧栏多出一个与「下载」平级的大类 | **并回下载页第一格**（本条） |

所以这次**删掉的是"独立页"这个外壳，不是向导**：`PageId` 里的 `'install'`、
`InstallGamePage.tsx`、侧栏那一格全都删了；`InstallComposer` 的 `variant='page'`
（两步 + 步骤条 + 右栏结论）**原样保留**，只是摆回下载页的第一个页签。
★ 也就是说「合并回去」没有把"繁乱"一起带回来 —— 每一屏仍然只回答一个问题。

#### 57.2　`DownloadTab` 里的 `'game'` 又写回来了

上一轮删它时的理由是"不许出现没人渲染的页签"，这一轮它是**真的**又有一格在渲染它 ——
同一个理由（页签必须有渲染方）决定了两次相反的写法。类型仍然在守这件事。

#### 57.3　所有"去装游戏"的入口统一走 `goDownloadTab('game')`

用户第 1 条「**转到下载页**」落在这里。改动的五个入口：

| 入口 | 之前（独立页那版） | 现在 |
|---|---|---|
| 版本列表「新装一个」 | `go('install')` | `goDownloadTab('game')` |
| 版本列表空状态「安装游戏」 | `go('install')` | `goDownloadTab('game')` |
| 启动页空状态「去安装游戏」 | `go('install')` | `goDownloadTab('game')` |
| 崩溃弹窗「重新安装游戏」 | `go('install')` | `goDownloadTab('game')`（文案改成"在「下载」页的「安装游戏」里…"） |
| 崩溃弹窗「重装加载器」 | `go('install')` | 同上 |
| 下载页空状态「去安装游戏」 | `go('install')` | `setDownloadTab('game')`（**同一页内**切格，不用跳页） |

★ 用 `goDownloadTab` 而不是"先 `go` 再点页签"：它是**一次派发到位**，
不存在"目标页还没挂载，事件就丢了"那个老问题（AppContext 里写着这段历史）。
★ 真机量过：点「新装一个」之后 `page-title` 是「下载」，高亮的页签是「安装游戏」，
而且落地的就是向导 —— 三件都对得上。

#### 57.4　「2.写「无」」：附加组件的短状态

那条**说错话**的红角标删了。它原来写「与当前加载器不兼容」，而真机上的两个例子是：

| 组件 | 真原因（`title` 里的整句） |
|---|---|
| OptiFine | 「OptiFine 确实没有发布 26.3 的版本」 |
| LiteLoader | 「LiteLoader 需要 Forge 作为基座，无法独立安装在原版上」 |

**两条都跟"加载器不兼容"不是一回事** —— 角标把两种不同的情况写成了一句具体的错话。
现在：卡片上只有**一行短状态「无」**，整句理由留在 `title`（悬停即见）。

★★ 这里**推翻了我 2026-09-15 自己定的一条**（当时我给"不兼容"单开一档显示「不兼容」，
理由是"想在字面上区分我们的限制与上游没有"）。用户明确不要这个区分。
**保留下来的是那条更硬的规矩**：已经勾上、后来被判不兼容的组件（`removed`）
仍然把整句写在卡片上 —— 那是"你明明选了它"，只写「无」会让人以为自己的选择凭空消失。

#### 57.5　真机验收（`live-install-page-check.mjs` 按最终形态重写，32 条全过）

* A 侧栏**没有**「安装游戏」了（合并的信号）
* B 下载页第一格是「安装游戏」、**默认就选中**、一共六格
* C 页头仍是「下载」（它不是独立页了）/ 默认在"选游戏版本"这一步 / 搜索框与渠道筛选 / 清单到手自动选最新正式版 / 两步 / 第 2 步未选时灰
* D 右栏结论卡的几何（清单 363px 高、右栏贴在其右侧、底色分层）
* E 第 2 步：加载器、版本名称、确认单、唯一安装按钮、翻页不丢选择
* **G 附加组件：选不了的两个短状态都是「无」，没有任何"不兼容"角标，理由仍在 title**
* **H 版本列表「新装一个」→ 落在下载页的「安装游戏」格（page=下载 / onTab=安装游戏 / 有向导）**
* I 弹窗形态（派发 `ieml:create`）仍是左右两栏，没被向导影响
* J 窄窗口 900px：右栏降级到清单下面
* 全程无异常；截图三张（第 1 步 / 第 2 步 / 弹窗）

### 五十八、再改向：点一行就进、模组加载器**整屏一页**（0.1.0-rc.1，2026-09-23 夜）

#### 58.1　用户这句话定了两件事

原话：**「像 mod 一样点击那行比较不错，然后给模组加载器像 mod 页那样单开一页」**。

| | 之前（§56/§57） | 现在 |
|---|---|---|
| 选版本 | 点一行 = 选中，再点「下一步」 | **点一行就进下一页**（与 Mod 页"点一行就装"同一手感） |
| 选加载器 | 同一页的第 2 步（右栏 + 步骤条） | **整屏自己的一页**：页头（`← 返回` + 标题）+ 正文 + 底部动作条 |

参照物就是 Mod 的安装页（`ResourceInstallPage`）：`page-head` + `← 返回` + 标题，
整宽的内容，动作在底部。所以这一屏**不再有下载页的页头与页签** ——
与整合包安装页用的是同一套"整屏"做法（§46），由 `onStepChange` 通知外层收起来。

#### 58.2　★ 步骤条**删掉了**（这是改向的必然结果，也是要请用户确认的一处）

步骤条说的是"现在在第几步"。而现在的答案是**页面本身**：
第 2 屏不是"同一页翻到第二步"，而是自己带页头的一页。

如果两样都留，就会出现"一条步骤条说第 1 步、页头却写着模组加载器"这种自相矛盾。
所以：删步骤条，在清单上方留**一句**话把交互说清楚 ——

> 点一行就进下一页（选模组加载器）—— 不选加载器就是纯原版

一句话替代一条步骤条，也更符合 ADR-054 的"一行说状态"。
★ 这条是**我的判断**，不是用户明说的（用户只说"像 mod 那样"）：留在这里备案，
用户下一轮说"步骤条要回来"就照做。

#### 58.3　第 1 屏顺带变简单：右栏整个没了

点一行就进下一页之后，"选中的版本"那张结论卡就没有存在意义了（你不可能"选中但不进"），
于是连同它的 Java/体积/盘上三行一起删掉 —— 那三条信息现在在这一屏的
**页头**（版本 + 渠道 + 日期 + 盘上有没有）与**底部动作条**（体积 + Java + 耗时）里，
一条都没丢。副作用是**版本清单拿到了整宽**：真机 300 → **934px**。

#### 58.4　★★ 真机抓到一个"只在几何里才看得见"的缺陷：版本名称被挤到折线以下

第 2 屏第一版把「版本名称」放在正文最后一块。真机上量出来：

| | |
|---|---|
| 正文可视高度 | **507px** |
| 加载器(261) + 附加组件(147) + 版本名称(81) + 间距 | **589px** |

于是那一块（含输入框，y=683）**在折线以下**，不滚动就看不见 —— 截图里那块是空白。

**更该记的是我的检查为什么没抓到它**：那条断言是
`nameValue.length > 0` —— 从 DOM 读到值就算过。**"在 DOM 里"不等于"看得见"**，
这和之前"401 证明接口存在"是同一类错：**判据选错了层**。
现在两处都改了：
* 实现：版本名称搬到**页头右侧**（`gw-name`），永远在屏幕上，不受窗口高度影响；
  输入框**只有一份**（`nameField`），弹窗形态仍然把它放在正文块里。
* 检查：加 `D7b`，判据是**几何** —— 输入框整个要落在 `window.innerHeight` 之内。

#### 58.5　真机验收（`live-install-page-check.mjs` 重写，35 条全过）

* A 侧栏没有「安装游戏」/ B 下载页第一格默认选中、六格 / C 页头是「下载」+ 搜索与渠道 + 那句提示 + 清单真的列出 + **整宽 934px** + **没有步骤条** + 有高度
* D 点一行 → 整屏页：标题「模组加载器」/ 写着"装到 26.3"/ 有「← 返回」/ **下载页页头与页签都收起来了** / 加载器选项 / **版本名称在视口里** / 底部动作条有体积与 Java 与安装按钮 / 铺满内容区
* E 附加组件短状态「无」、无"不兼容"角标、理由仍在 `title`
* F 「返回」回到清单页、页签回来、**刚才那行仍是选中态**
* G 窄窗口 900px：加载器行退回竖排、**没有横向溢出**
* H 版本列表「新装一个」→ 下载页 + 安装游戏格 + 清单
* I 弹窗形态仍是左右两栏（`gw-full` 不存在）
* 全程无异常；截图两张（清单页 / 模组加载器页）+ 弹窗一张

#### 58.6　这一轮把样式也清了一遍

上一版那一整块 `.gw-*`（步骤条 12 条规则 + 右栏 + `.gw-facts` 表格）**没有渲染方了**，
整块删掉（259 行 → 65 行）。★ 替换用脚本按起止标记切片，并自检
"标记各出现一次 + 花括号配平"—— 手抄 259 行 CSS 在这个仓库已经出过事。

### 五十九、「愚人节」那一档的**分类莫名其妙**：又是"同一判据写两遍"（2026-09-23 夜）

#### 59.1　用户给的截图说了什么

用户选了「愚人节」档，得到三个分组：**其他（2 个）/ 愚人节（4 个）/ 快照（2 个）**，
一句「**这个分类就有点莫名其妙了**」。

他看到的确实是错的 —— 那 8 个**全都是**愚人节版本（日期全是 4 月 1 日前后），
却被切成了三类。

#### 59.2　根因：愚人节的名单有**两份**，而且其中一份排在快照规则后面

| 位置 | 内容 |
|---|---|
| `domain/loader-caps.ts` | `APRIL_FOOLS`，**9 个** id —— 权威那份，还写着"宁可漏一个，不可错一个"的理由 |
| `components/VersionIcon.tsx` | **另抄了一份，只有 4 个**，而且判在快照规则**之后** |

后果正好解释了截图：

| 版本 | 被哪条规则接住 | 结果 |
|---|---|---|
| `23w13a_or_b` / `22w13oneblockatatime` / `3D Shareware v1.34` / `20w14infinite` | 那份 4 个的白名单 | 「愚人节」4 个 ✓（碰巧对） |
| `15w14a` | 快照正则 `^\d{2}w\d{2}[a-z]$` | ❌「快照」 |
| `1.RV-Pre1` | 快照正则 `-(pre\|rc)\d*$`（`-Pre1` 命中） | ❌「快照」 |
| `24w14potato` / `25w14craftmine` | 谁都不认 | ❌「其他」 |

★ 这正是 ADR-051 那条"**同一个判据写两遍**"的老毛病 —— 第 N 次。
而且它比一般的那种更难发现：两份名单都不报错，**只在界面上以"分组标题"的形式露出来**。

#### 59.3　决定

1. **只留一份**：`versionFamily()` 直接调 `isAprilFoolsVersion()`（`domain/loader-caps.ts`），
   并且**排在快照规则之前**判 —— 愚人节版本长得就像快照，顺序反了就会被吞掉。
2. **这一档不分组**：整档本来就是同一类东西，再按"世代"切只会切出莫名其妙的标题。
   愚人节版本总共不到十个，也不需要"折叠"这种管理 900 个版本的手段。
   → `channel === 'fools'` 时**平铺**渲染，一个组标题都不出现。
3. **行的实现只有一处**：把那一行（`.wz-item` 按钮，66 行）从分组渲染里**搬出来**
   变成 `versionRow(v)`，分组与平铺共用 —— 两种排布不会漂移。
   ★ 搬运用脚本（按标记取段 + 去缩进 + **去缩进后逐字节比对**），不手抄。

#### 59.4　真机验收（`live-install-page-check.mjs` 新增 K1–K8，44 条全过）

| | 断言 | 实测 |
|---|---|---|
| K1 | 愚人节档**一个组标题都没有** | `[]` ✓ |
| K2 | 列出了版本 | 8 行 ✓ |
| K3 | **每一行**的图标都是「愚人节」配色 | 8/8 都是 `vi-april` ✓ |
| K4 | 原来被错标的四个都在这一档里 | `15w14a` / `1.RV-Pre1` / `24w14potato` / `25w14craftmine` ✓ |
| K5 | 快照档里没有愚人节版本混进来 | 0 行是 april 配色 ✓ |
| K6 | 快照档**仍然**分组（世代分组在那里是有用的） | `26.4 / 26.3 / 26.2 …` ✓ |
| K7 | 「全部」里搜 `15w14a`，图标是愚人节配色 | ✓（以前被当成快照） |
| K8 | 它的组标题写「愚人节」 | `愚人节 1 个` ✓ |

截图（`愚人节档-平铺.png`）里那 8 行的日期全是 4 月 1 日前后 —— 用户一眼能对上。

### 六十、「两个返回按键？」——同一个动作两个入口（2026-09-23 夜）

用户看着「模组加载器」那一页问：**「两个返回按键？」**。截图上确实是两个：

* 页头左上 `← 返回`
* 底部动作条 `← 换一个版本`

**两者作用完全一样**（都是 `setStep('version')`），只是措辞不同 —— 一个说"回去"，
一个说"换个版本"。这就是用户在别处已经指出过一次的那类问题
（账号弹窗的「两个关闭键」）：**同一个动作不该有两个入口**。

决定：**删掉底部那一个**，保留页头左上角那个。理由是位置与参照物：

* 左上角回退是**页面级**约定位置（Mod 的安装页 `ResourceInstallPage` 就是
  `← 返回` + 标题，而这一页正是照它做的）；
* 底部动作条是"确认 + 执行"的地方，**只留一个按钮**（安装）最干净。

★ 顺带记一个**同类但没动**的地方：`footBar`（创建版本**弹窗**用）里也有一个「取消」，
而弹窗右上角本来就有 ✕ —— 与账号弹窗那两个关闭键是同一个毛病。
弹窗那条路在应用里走不到（§56.7 记过），所以这一轮没顺手改：
**要么整条删掉，要么给它入口时一起收拾**，不单独改一半。

真机验收加了一条判据（`D4b`）：整页里"回退类"按钮**只能有一个**，
并把它们的文案打出来（`["返回"]`）—— 这条断言是"数个数"，正是这类缺陷该有的判据。

### 六十一、「这个底黑的效果好诡异」：**从弹窗继承来的实心色带**（2026-09-24）

用户看着底部动作条：「**这个底黑的视觉效果好诡异啊**」。

#### 61.1　这条带子是从哪来的

`.cw-foot` 是**弹窗**那边的零件（创建版本弹窗的底部条），它铺的是
`background: var(--bg-surface)` —— 而弹窗的正文**也是**同一个底色，
所以那条带子在弹窗里**根本看不见**（它只是把底色又铺了一遍）。

搬到一张**玻璃整页**上就露馅了：页面底色是半透明的玻璃层，
而这条是**实心**深色，于是它横切过去，左边缘与内容齐平、又没有圆角 ——
看起来就像一块浮在玻璃上的方块。

★ 这是"**零件换个地方用，它带来的样式就变成了噪音**"的一个实例：
组件复用好，但**它自带的视觉**不一定跟着搬。真机截图才看得出来（代码里两处都"对"）。

#### 61.2　决定

`background: none` —— **不铺色**。这一行只是"摘要 + 一个按钮"，
靠 `.cw-foot` 自带的 `border-top`（发丝线，与页面别处的分隔同一种）与正文分开就够了。
横向内边距仍然是 0，让它与页头、正文左边缘对齐。
...
### 六十四、修复批（三）：**A-1 三处「启动」按钮点了什么都不发生**（2026-09-24）

#### 64.1　根因：监听器住在"那一刻根本没挂载"的页面里

「启动」这个动作用的是跨页面的自定义事件：派发方
`window.dispatchEvent(new CustomEvent('ieml:launch-request', { detail: id }))`
（三处派发点：`VersionsPage.tsx` 的行菜单、`InstanceOverview.tsx` 的概览「启动」、
`AppShell.tsx` 安装完成后的那一下）。

而唯一的监听方原来挂在 **`LaunchPage`** 里 —— 那个组件只在 `state.page === 'launch'`
时才渲染。而这三个按钮**全都在别的页面上**点。于是事件发出去时，
**现场一个监听者都没有**：不跳页、不启动、不报错，什么都看不出来。

真机判据（`probe-a1-fixed.mjs` 第 ① 条）：在版本列表页上数
`getEventListeners(window)['ieml:launch-request']` —— **修之前是 0**。

#### 64.2　修法：监听器搬到**常驻外壳**上，并且只能有一个

`AppShell`（任何页面下都挂载）接管监听：`setLaunchTarget(id)` → `go('launch')`
→ 轮询等 `#ieml-launch-btn` 出现（100ms 一次，最多 2 秒）→ 点它。

★ **启动页里那份监听器必须删掉**（已删，原位留了说明注释）。
留着就会变成"两个监听者各启动一次"—— 那是同一个 bug 的镜像版本，
而且是最难发现的那种坏法：界面看起来正常，只是每次点都启动两回。

#### 64.3　真机验收（`tools/live/probe-a1-fixed.mjs`，debug exe）

```
① 版本列表页上 ieml:launch-request 监听器 = 1
② 点了行菜单的「启动」=true；页="启动" 文本里含实例名=true
③ 沙盒启动日志：["probe-a1-1790188125.log"]   沙盒 java 进程数 = 1
④ 指向真实根 D:\IEML 的 java = 0
```

★ **这条探针自己踩的坑值得单独记**：它的判据 ③ 原来是
「沙盒里没有游戏文件 ⇒ 启动必然失败 ⇒ 出现失败提示就是"流程跑到了"的证据」。
实测**前提就是错的**：缺文件时启动流程会先下载（约 208 MB）**再真的把游戏跑起来**。
于是那次运行里判据 ③④ 全红 —— 而 A-1 **其实已经修好了**。
（副产品：沙盒里留下了 `logs/probe-a1-*.log`、`natives/*.dll` 和一个活着的 java 进程，
它锁住文件，导致下一次运行连沙盒都删不掉，报 `EBUSY`。
现在的探针会先杀掉**命令行里带本沙盒路径**的 java 再清理 —— 绝不动别人的游戏进程。）

★ 一条通用的教训：**"这个操作必然会失败"这种前提，本身就是一条未经检验的判据**。
它错了，红绿就全反了。

### 六十五、修复批（四）：**A-4 清单/偏好还住在游戏根目录**（2026-09-24）

#### 65.1　原状：数据目录搬出游戏根目录，**只搬了一半**
...
| `prefs.json` | 游戏根目录 | `own_root` |
| `ms_client_id.txt` | 游戏根目录 | `own_root` |
| `cf_api_key.txt` | 游戏根目录 | `own_root` |

真机后果（这一台）：`D:\IEML\instances.json`（mtime 今天 17:25，3 个实例）是应用
**一直在写**的那份；`%APPDATA%\IEML\instances.json` 是 **09-13 的陈旧副本**。
于是：

* 用户在「候选盘」那页点删除 `D:\IEML` → **实例清单 + 全部设置一起没**，
  而两道确认框里一个字都没提这件事；
* 两处各有一份、谁看谁糊涂 —— 我在缺陷报告里就据那份陈旧副本误判过一次
  （以为"主题没恢复"，其实是看了 `%APPDATA%` 那份 `dark`）。

★ 还有一段**陈旧的注释**（`lib.rs`）：它写着"版本、Java、缓存、日志都还住在这个根目录里，
  整体搬迁要等用户同意"——而 09-23 用户已经同意、目录也搬完了，只剩这四个文件。
  **注释比代码先过期，而读代码的人（我）会信注释。**

#### 65.2　修法

1. `AppPaths` 新增 `own_file(name)` / `own_file_for_read(name)` / `legacy_record_file(name)`
   以及 `instances_file()` / `prefs_file()` —— 路径只有这一个来源。
2. **写**一律去 `own_root`；**读**优先 `own_root`，那儿没有才回退游戏根目录的老位置
   （老用户升级当次就能读到自己的东西，不依赖复制是否成功）。
3. 启动时 `platform::adopt_records(&paths)` 做一次性"收养"，规矩三条：
   * **只复制、绝不删源**（用户随时能退回旧版启动器）；
   * 目标已有、且**不比源旧** → 一个字节都不动；
   * 真要覆盖目标之前，先备份成 `<名字>.bak`。
4. ★ 收养判据里**必须有"源比目标新"这一条**：这正是本机实测出来的情形
   （游戏根那份是活的、`%APPDATA%` 那份是 09-13 的死副本）。
   若只按"目标存在就跳过"，用户最近十天的改动会被死副本永远盖住。
5. 前后端一起改：`commands.rs`（instances/prefs）、`auth`（client_id）、
   `net::curseforge`（api key）。清空 client_id / API key 时**连老位置那份一起删** ——
   否则下次启动会被收养回来，用户会觉得"清空没用"。
6. 「删除游戏根目录」的第一道确认里**写明不会连累什么**：
   「**不会**动到：启动器的实例清单、设置、登录用的 client_id、CurseForge Key」。
   （危险动作的确认框必须把"删什么 / 不删什么"都说清，而不是只说"不可恢复"。）

#### 65.3　真机验收

**（a）沙盒**：`tools/live/probe-a4-fixed.mjs`（debug 与发行版各跑一遍）——
造一个"老用户"沙盒（文件全在游戏根目录里），启动 → 收养 → **把整个游戏根目录删掉** → 重启：

```
① 老文件被收养进 own_root（实例名 ["探针A4·甲","探针A4·乙"]）      ✓
② 偏好也搬过去了：own/prefs.theme="daiqing" 界面="daiqing"        ✓
③ 启动器不再往游戏根目录写：根里 mtime 未变=true                  ✓
④ 删掉游戏根之后重启：实例清单还在（版本列表 2 行）                ✓
④b 设置也还在：data-theme="daiqing"                              ✓
```

**（b）真实数据（本机 `D:\IEML` + `%APPDATA%\IEML`）**：
用**桌面那份发行版 exe** 启动一次，前后对照：

| 文件 | 启动前 | 启动后 |
|---|---|---|
| `D:\IEML\instances.json` | 17:25:15（3 个实例） | **17:25:15（一动没动）** |
| `D:\IEML\prefs.json` | 17:25:15（daiqing） | **17:25:15（一动没动）** |
| `%APPDATA%\IEML\instances.json` | 09-13 16:00（3 个实例） | 19:13:22（**活的那份**） |
| `%APPDATA%\IEML\prefs.json` | 09-13 16:00（theme **dark**） | 19:13:22（theme **daiqing**） |
| `%APPDATA%\IEML\prefs.json.bak` | （不在） | 09-13 16:00（theme dark，**旧的没丢**） |

界面 `data-theme="daiqing"` —— 也就是说：**主题从来就是对的**，
错的只是"它住在哪、哪份才算数"。
...
### 六十八、修复批（七）：**B-1 更新 Mod 时说"已替换"，旧 jar 一个都没删**（2026-09-24）

#### 68.1　原状

`ModsPanel.applyUpdates()` 对每个勾选的更新只做一件事：
`installMod(url, file_name, slug)` —— 而 Rust 的 `install_mod` 就是
`create_dir_all` + `download_one`，**没有任何 remove/rename**。
Modrinth 每次的 `filename` 通常带版本号（`sodium-0.5.11.jar` ≠ `sodium-0.5.8.jar`），
于是更新一次 = `mods/` 里同时躺着新旧两份，而提示写着
「**N 个 Mod 已替换为新版本**」。

★ 这类缺陷的形状值得记：**不是"功能没做"，而是"话说了、事没做"** ——
用户按提示理解（旧的不在了），现实是旧的还在，而后果（同一个 Mod 两份）由他承担。

#### 68.2　修法：判据收成一处纯函数，而且**故意收得很窄**

`domain/mods.ts::oldFilesToDrop(entries, oldSha1, newFileName)`：

| 规矩 | 为什么 |
|---|---|
| 只认**同一个 sha1** 的文件 | sha1 是"这次要更新的那个旧文件"的**身份**（来自在线库反查），不是猜的 |
| 新文件名与旧文件**同名 → 不删** | 同名时 `installMod` 直接覆盖，这时去删就等于把刚下下来的新文件删掉 |
| **不用** displayName / 前缀相似度 | "看起来像同一个 Mod"的猜测会误删用户手放的其它版本（这个仓库为"猜"付过代价） |
| 空 sha1 → **一个都不删** | "不知道要更新谁"时，宁可留下旧文件 |
| 删除走**系统回收站**（`permanent=false`） | 判据万一错了，用户还能捞回来 |

提示也一起改了：成功时如实说
「N 个 Mod 已替换为新版本；M 个旧文件已移入系统回收站（删错了还能捞回来）」；
旧文件删不掉时**不谎报成功** —— 新文件确实装好了，但失败原因会一并列出来。

#### 68.3　验收（以及**没验到**的部分，如实说）

* 单测三条（`tests/domain.test.js`，跟着既有「领域单元测试」门禁跑）：
  只删同 sha1 / 同名不删（含大小写）/ 同 sha1 多份全删 / 空 sha1 不删。
* **端到端真机复现仍然没做到**：它需要"同一个 Mod 有两个版本、版本号与实例的
  MC + 加载器对得上、且文件没被作者禁止第三方下载"。第 2/3 轮试过两次
  （`probe-bug-repro-7.mjs`），一次点错元素（卡片本身没有 onClick），
...
#### 70.6　发布 rc.2（2026-09-24）

用户先说的是「重写一遍更新日志的 rc1 更新报告，然后正式发布 rc1」。
查证后发现 **rc.1 在 09-21 就已经发布过**（线上端点自检：`version=0.1.0-rc.1`、
Ed25519 验签通过），而更新检查是**按版本号比较**的 —— 再发一次 rc.1，
已经装了 rc.1 的人**收不到更新**（同版本 = "已是最新"）。
所以把选择摆给用户（升 rc.2 / 覆盖 rc.1 / 先只写说明），用户选了**升 rc.2**。

于是：

1. `node tools/set-version.mjs 0.1.0-rc.2`（6 个文件 7 处版本号 + 文档口径都由它校验）；
2. `CHANGELOG.md` **新写一节** `0.1.0-rc.2`（rc.1 那节按规矩一字未动）——
   它就是用户在更新提示里看到的那份说明；
3. 构建签名发行版 + 部署桌面；
4. **发布链路按顺序走完**（顺序很重要，第一次我把自检放在了生成清单之前，它读的是上一次的清单）：

```
node tools/release/publish-cnb.mjs            # 只打包 + 生成 latest.json（不上传）
node tools/release/verify-manifest.mjs        # 上传前自检 → 16 项全绿
node tools/release/publish-cnb.mjs --upload   # 真上传
node tools/release/verify-endpoint.mjs        # 上传后**按客户端方式**验
node tools/release/check-exe-wiring.mjs       # exe 里的更新能力
```

真机结果：

```
线上端点    HTTP 200  version=0.1.0-rc.2
匿名下包    3.40 MB
验签        keyId=8B66ACD053761632 · Ed25519 通过（客户端能装上）
exe 接线    端点 / 公钥 / 插件 / ACL / installMode 五项都在
产物        ieml.exe 9,042,944 字节
            sha256 CED055D0510F05425C0903EB4612E02C97F763066A7870D8B1C2F6CAC148DAE2
安装包      IEML_0.1.0-rc.2_x64-setup.exe（3.40 MB）+ .sig（420 字节）
```

★ 还补了最后一道**用户视角**的确认（`tools/live/probe-live-update-check.mjs`）：
拿**装到桌面的那份 rc.2** 启动 → 关于页自报 `0.1.0-rc.2`，
点「检查更新」→ 「已是最新版本」。
这一步才真正证明"端点可达 + 验签 + 解析"整条链在**交付物**里是通的
（任一环坏了，那句话会变成「检查更新失败：…」）。

#### 70.7　rc.2 发布之后：用户拿掉两个重复的启动键（2026-09-24）

用户发来实例概览页的截图 +「**这两个启动键都不要**」：
右上角那个「启动」与左侧栏底部那个「启动这个版本」。

★ 这**不是**缺陷修复，是用户对界面的取舍 —— 同一个动作在界面上有三处入口
（版本列表行菜单、实例概览页头、实例二级页侧栏），他只想留版本列表那一处。
按用户要求删掉这两处（`InstanceOverview` 页头那个「启动 / 停止游戏」、
`AppShell` 实例侧栏底部那个「启动这个版本 / 停止游戏」），
并把随之出现的死变量（`isRunning` / `isInstanceRunning` 的 import）与死 CSS
（`.side-launch` 两条规则）一起清掉。

★ **能力没有删，只是入口去重**：真机判据里专门留了一条对照组 ——
版本列表那一行的 `⋯` 菜单里**仍然有**「启动」
...
#### 70.9　资源下载页的「时代分类」字号放大一档（2026-09-24）

用户：「资源下载里的**时代分类**都大一点，目前有点小有点费眼」。
"时代分类"就是那排**大版本分组**（chips + 分组标题）。改前的值直接从 CSS 里读出来：

| 元素 | 改前 | 改后 |
|---|---|---|
| 版本 chips（「全部 472 / 26.3 3 / 1.21 168 …」） | 12px，内边距 4/11 | **13px**，内边距 6/13 |
| 分组标题整行 | 13px，内边距 7px | **14px**，内边距 9px |
| 分组标题里的版本号（等宽） | 继承 13px | **14px** + `letter-spacing: 0.01em`（等宽字体同字号看着更小） |
| 分组右侧「N 个版本」 | 12px | **13px** |
| 顶部「X 的全部版本（N 个）」 | 12px | **13px** |

★ 作用域只管资源/整合包那一套（`.res-vchips` / `.res-vgroup-*` / `.res-versions-head`），
**不动全局 `.chip`** —— 别的页面上的徽标不该跟着变大。
★ 判据（`tools/live/probe-res-groups-size.mjs`）量的是**真机算出来的字号**，
不是读 CSS 源码，并带一条对照（版本行里的版本号**不该**变，实测仍 13px）：

```
① chips 12 → 13 ✓   ② 分组标题 13 → 14 ✓   ②b 组名（等宽）14 ✓
③ 「N 个版本」12 → 13 ✓   ④ 顶部那行 12 → 13 ✓
```

★★ **两条探针都在这里栽过一次，值得记**：我两次都在整合包列表**还停在骨架屏**时就去点卡片
（`.pack-card-sk` 有 6 个，真卡片 0 个），于是"没有卡片"、量到的全是 null ——
而**截图看起来完全正常**（截图是几十秒后拍的，那时列表已经加载好了）。
排查结论：这台机器上 Modrinth 的搜索有时要几十秒。
现在探针等的是 `.pack-card:not(.pack-card-sk)`（**真卡片**）并且给到 90 秒。
**"看起来正常"的截图不能替代"点到了东西"的断言。**

#### 70.10　同一档字号也用到「安装游戏」页（用户：「这个也要放大」）

用户接着发了「安装游戏」页的截图：「**这个也要放大**」。
那边是另一套类名（`InstallComposer`），所以按同样的思路**作用域限定在 `.gw-col`**
（版本清单那一列）——`.seg-sm` / `.wz-item` 这些类在别处也在用，不能全局改：

| 元素 | 改前 | 改后 |
|---|---|---|
| 渠道分段控件（正式版 / 快照 / 愚人节 / 全部） | 11px，高 23px | **13px**，高 27px |
| 世代标题（26.3 / 26.2 …） | 12px | **13px** |
| 标题右侧「N 个」 | 12px | **13px** |
| 版本行里的版本号 | 13px | **14px** |
| 版本行下面那行小字（日期 · 盘上有什么） | 11px | **12px** |
| 「点一行就进下一页…」那句提示 | 11px | **12px** |

判据（`tools/live/probe-install-list-size.mjs`，同样量真机算出来的字号）：
六条全绿，并带一条对照 —— **侧栏导航项仍是 13px**（证明改动确实只落在 `.gw-col` 里）。

#### 70.11　发布 rc.3 时抓到的**发布工具 bug**：更新说明只推了标题那一行

用户：「编写更新日志，推送最新更新」。写好的 rc.3 那一节 2410 字，
按流程 `publish-cnb.mjs` → `verify-manifest.mjs` → `--upload` → `verify-endpoint.mjs`
一路全绿 —— 但上传后我顺手读了线上 `latest.json`：

```
notes 长度 = 85 字     ← 就是 `## 0.1.0-rc.3 — …` 那一行
```
...
readFileSync('CHANGELOG.md','utf8')
  .match(new RegExp(`^## ${ver}[\\s\\S]*?(?=\\n## |$)`, 'm'))   // ← `m` 标志
```

带 `m` 时 `$` 匹配的是**行尾**，于是惰性匹配 `*?` 立刻满足，停在第一行。
去掉 `m`（`^` 只认串首、`$` 只认串尾）并用 `(?:^|\n)` 兜住"标题在行首"，就对了。

★★ **为什么它能藏这么久**：自检里那三条关于 notes 的断言
（无乱码 / 是 CHANGELOG 的一节 / 提到了版本号）**在"只有标题"时全都成立** ——
**判据太弱，等于没有**。这正是这个仓库反复记的那类问题的镜像：
不是没检查，而是检查的东西**证明不了要证明的事**。

修法与加严：

1. `publish-cnb.mjs` 修正则，并顺手去掉尾部那条 `---`（排版用的，不该进更新说明）；
2. `verify-manifest.mjs` 新增两条**会红**的判据：
   * notes 正文 ≥ 200 字（实测：修好前 85 字、修好后 2410 字）；
   * notes 至少 3 行非空正文行。
   并且**先证明它能红**：把 notes 手动截成一行跑自检 → 两条如期报错 ✓；
3. 重新生成清单并上传；线上现在是 2410 字 / 90 行 / 6 个小标题，无编码问题
   （用 node 读的 —— PowerShell 的 `Invoke-RestMethod` 会把它显示成乱码，
   那是**显示**问题，不是数据问题，别被它带偏）。

★ 一个如实说明：**应用界面目前不显示 notes**（`useLauncherUpdate` 存了 `state.notes`，
但没有任何组件渲染它）—— 所以这份更新说明现在只在发布页/接口里看得到。
要在启动器里给用户看（例如顶栏角标 hover 或关于页）是**新功能**，没做。

#### 70.12　用户报的三件事：更新按钮 / 更新日志页 / 启动时的更新提示（2026-09-24）

**① 关于页的按钮：下好之后点它又去"检查更新"，还再下一份**

现场（读了代码就明白）：那个按钮的**文字**会随着状态变（`ready` → 「重启并更新」），
而 `onClick` **无论什么状态都调 `checkNow()`** —— 于是它又去问一次服务端、
拿到一个**新的 Update 对象**放进 `pending.current`，接着 `download()` 从头再下一份。

修法两处（缺一不可）：

* `AboutPage`：按钮按状态分支 —— `ready` → `install()`；`downloading`/`installing` → 什么都不做
  （禁用 + 显示进度/「正在安装…」）；其余才 `checkNow()`；
* `useLauncherUpdate`：新增 `downloaded` ref 并加守卫 —— **手里已经有一个下好的包，
  就不许再查、更不许再下**，只把状态重申成 `ready`。
  ★ 守卫必须在 hook 里：界面之外（角标、快捷键、将来别的入口）也可能调进来。

**② 「更新日志一篇没更新，rc1 的也没重写」**

根因不是我没写 CHANGELOG —— 是**应用里的「更新日志」页看的是另一份文件**：
`src/data/release-notes.ts`（面向用户的"说人话"版，与仓库的 `CHANGELOG.md` 分开维护，
理由写在那个文件头部）。它的最新一条停在 `0.1.0-rc.1（未发布）`（09-22），
rc.2 / rc.3 从来没写进去 ⇒ 用户看到的就是"一篇没更新"。

处理（这份文件是**用户向文案**，不是"已发布的工程记录"，所以可以改）：

* 补 `0.1.0-rc.2`（把 09-22 起那批界面修复 + 这一轮缺陷排查的修复合在一起 ——
  它们**确实**都是在 rc.2 这一版到用户手上的）与 `0.1.0-rc.3`（今天这四条）两条；
* 给 `0.1.0-rc.1` 一条**如实**的：这一版没有功能改动，是"进 rc 的五条判据"；
* ★ 顺手把报告里那条**没能复现**的低项定位到了 —— 它就在这份文件里：
  * `beta.55`：「绿宝石皮肤文件的导入与导出（皮肤页）」—— 全仓**没有这个功能**，
    也没有"皮肤页"（皮肤是账号面板里按 Mojang 官方接口拉真实皮肤、本地裁头部）；
  * 旧 `rc.1` 那条：「版本名统一成「Minecraft 1.12.2 ：原版」这种写法」——
    界面上写的是「1.20.1 · 原版」（没有 "Minecraft" 前缀、也没有全角冒号）。
  两处都按事实改掉，并在文件头写了"这份同样受不许有假承诺的约束，写之前去界面核一眼"。

**③ 要求的新行为：启动时静默检查 + 有更新就弹窗 + 「关于」右边标「有更新」**

静默检查本来就有（开机 8 秒后自动查一次、失败不留痕），补的是**看得见**的两处：

* 侧栏「关于」按钮里加 `<em className="side-badge">有更新</em>`（用户点名要的位置与文案）；
* 查到就弹一次窗（`AppShell` 里新加的 Modal）：文案随阶段变（下载中 / 已下好），
  按钮「现在重启并更新」（只有 `ready` 时可点）、「稍后」、「看看改了什么」（跳更新日志页）；
  关掉之后**本会话不再自动弹**，但侧栏那个标记一直在。

**真机验证（`tools/live/probe-update-flow.mjs`）**——怎么"可控地"造出一个更新：
线上是 rc.3，所以临时把版本号降到 rc.2 构建一个 debug exe（跑完立刻改回 rc.3 并复查一致性），
对它来说 rc.3 就是**真更新**（真端点、真验签、真下载）。五条判据：

```
① 侧栏「关于」右边写着「有更新」 ✓
② 查到就自动弹窗（「发现新版本 0.1.0-rc.3」）✓
③ 下好后弹窗主按钮可点、文字是「现在重启并更新」✓
④ 关于页按钮是「重启并更新」而不是「检查更新」✓
⑤ 点它 → 状态行变成「正在交给安装程序（启动器会退出并重开）」✓
   （坏行为会是「正在检查…」然后重新下一份）
```

★★ 两件必须记下来的事：

1. **⑤ 那一次真的把安装程序跑起来了**：Tauri 在下完之后**并不落盘**（包在内存里），
   只有点安装那一刻才写到 `%LOCALAPPDATA%\Temp\IEML-<版本>-updater-<随机>\…-installer.exe`
   并运行。我第一版探针想"先把包挪走再点"，结果什么都没挪到，于是 rc.3 被静默装到了
   `%LOCALAPPDATA%\IEML`（还建了桌面/开始菜单快捷方式，版本资源 = 0.1.0-rc.3 ✓）。
   —— 这恰好是这条链路**端到端**的最强证据（更新包 → 静默安装 → 装好），
   但探针不该每次都动用户机器 ⇒ 这一步现在**默认不点**，要验得显式设
   `IEML_PROBE_ALLOW_INSTALL=1`，并且点完立刻把安装程序与启动器收掉。
2. 顺手发现一处**未完全解释**的观察（如实记）：已安装那份 exe 与我的构建
...
★★ 中间踩到一次**探针自己造成的假红**，如实记（教训：判据红了先怀疑量法）：

* 事后我又跑了一遍"旧版 vs 部署版"的对照，结果自相矛盾：部署版 exe 的 sha256
  明明是修复版（`3B8FA6CD…DF47`，逐个核对过），却报出 C 盘路径、判据 ②③ 假红。
* 根因在 `probe-instance-root.mjs`：两个阶段**共用同一个 CDP 端口**。启动器是单实例的 ——
  旧进程没被彻底清干净时，第二阶段起的那个新进程会被单实例挡掉、悄悄退出，
  而探针照样连得上那个端口（那是**旧进程**还在服务）⇒ 它把旧构建的答案当成了新构建的。
* 修法：每个阶段各用一个端口（第二阶段的端口只可能由第二阶段的进程服务）；
  起之前等 `ieml` 进程真的消失（≤20 秒），起之后确认端口活了；连不上就明确报
  「本次测量无效」并**非零退出**，不拿上一条连接凑数。修完立刻复现了真相
  （报「连不上 CDP（new，端口 9972）」= 那次新进程根本没起来）。
* 真正定案用的是**单实例、不做 A/B** 的那条探针（`probe-versions-page-dir.mjs`，扩成逐行）：
  版本列表 3 行 → 三个游戏目录全部落在 `D:\IEML\instances\…`、且都存在；「打开目录」的
  toast 也是 D 盘 ✓。
...
### 七十三、换页面不回到顶部：`scrollTop` 是**那个 DOM 节点**的状态（2026-09-24）

用户（截图：侧栏 启动 / 版本列表 / 下载 / 设置 / 更新日志 / 关于）：

> 点击左侧栏切换页面时，不会回到最顶端，而是从上一页面继承当时的位置
...
```tsx
<main className="content">        {/* ← 滚动容器（overflow-y: auto） */}
...
换页只是把它**里面的组件**换掉，而 `scrollTop` 挂在 `<main>` 自己身上 ——
浏览器没有任何理由替我们归零，所以位置就留下来了。
（这不是"React 的锅"，是"谁拥有那个状态"的问题：滚动位置属于容器，而容器没换。）

#### 73.2　改法

`AppShell` 里加一个 ref + 一个 `useLayoutEffect`：
...
1. **`useLayoutEffect` 而不是 `useEffect`**：后者在浏览器画完之后才跑 ——
   新页面会先以"上一页那个滚动位置"出现一帧，然后才跳回顶部（肉眼可见的闪）。
   放在 layout 阶段（同一次绘制之前）就没有这一下。
2. **依赖里放的是"这一屏的身份"**：一级页面 / 打开的版本 / 二级页面 / 下载页页签 /
   资源安装页的目标。任何一个变了都算换了屏。
   ★ 反过来，**同一屏内的操作不进依赖**：版本列表换筛选、设置页拨开关、
   搜索框打字都不该把用户拽回顶部（这条在探针里有专门的反面判据）。
3. **不做"返回时恢复原位置"**：用户要的是"回到最顶端"。恢复原位置是另一种交互
   （要按屏记位置），没做 —— 如果以后想要，那是**新功能**，别顺手改判据。

#### 73.3　真机验证（`tools/live/probe-page-scroll-top.mjs`）

★ 判据挑的是**长页 → 长页**：如果目标页比源页短，浏览器会自己把 `scrollTop`
夹到新页的最大值（常常正好是 0）—— 那样这条判据在**坏构建上也会绿**，什么都证明不了。
（第一版就踩了：设置 906 → 版本列表 直接是 0，看着像已经修好了。）
...
坏构建（部署中那份，没有这个修复）：
  ① 设置(906) → 更新日志：实际 906   ✗
  ② 更新日志(1610) → 设置：实际 906  ✗
好构建（加了 useLayoutEffect 的那份）：
  ① 设置(906) → 更新日志：实际 0     ✓
  ② 更新日志(1610) → 设置：实际 0    ✓
  ③ 实例「设置」(396) → 「概览」：0   ✓（烟雾判据：概览页短，浏览器自己会夹到 0）
  ④ 同一屏内拨开关：906 → 890        ✓（点靠边缘的控件会被浏览器滚进视野几像素，
                                        那是浏览器行为，不是"换页归零"）
  ⑤ 三个页面都确实滚得动（906 / 1610 / 396）—— 防止 ①②③ 变成"永远为 0"的假绿
```

`tsc` 干净、`node tools/verify.mjs` 24 项全过。

★ 如实记：这一条**没有**进 BUG-REPORT 的原始清单（它是用户今天用出来的新问题），
所以只有 ADR 与探针留证；版本号未动、未发布。

### 七十四、发布 rc.4 + 更新日志的格式**定为必读模板**（2026-09-24）

用户两句话：「**更新日志的正文，我要这个格式，重编 rc1,rc2,rc3 的更新日志；你先给我模拟一下**」
和看过模拟之后的「**发布 rc4**」。

#### 74.1　格式（这是**模板本身**，以后每一版照这个写）

```
新增了 / 修复了 / 优化了 / 删除了 / 修改了      ← 顺序即显示顺序；没有内容的段**不写**
```

三条硬规矩（用户逐条点过）：

1. **每条开头重复一遍类别词** —— 段标题写「修复了」，条目就写「修复了 下载页的标题栏被封面糊住」。
   理由（用户点名要的）：单摘一条出去也知道它属于哪一类，不必回头看段标题。
2. **言简意赅**：一条一件事、一句话说完；**不写括号里的解释与本条的好处** ——
   用户明确划掉了两处：「（玻璃只做透明和折射，不给内容染色）」「（动效与视效一键降档，关掉还回你原来的选择）」。
   另外用户把「"啪"地消失」改成「直接消失」、「假警报」改成「假报错」—— 都用最直白的说法。
3. **不写假话**：没做的、只在某一台上做过一次的、只是"不再复现"的，都不许写成"已修复"。

落地：

* `src/data/release-notes.ts` 文件头现在就是这份模板（**必读篇目**），
  并把段名收进类型 `NoteGroupTitle` —— 段名写错会被 `tsc` 直接拦住；
* **按这个格式重编了 rc.1 / rc.2 / rc.3**，以及更早的 beta.53–beta.57；
* 用户指出的那条**挪了位置**：「关于页更新按钮下好后又重复下载」原本挂在 rc.3 里，
  可它是 rc.3 发布**之后**才修的 ⇒ 挪进新的 rc.4 一节；
* 新增 rc.4 一节（本轮六件事）。

★ 顺带把格式变成**会红的判据**：`tools/check-release-notes.mjs` 进 `tools/verify.mjs`（第 25 项）。
它查五件事：段名合法、**同一版本内段落顺序**、每段非空、每条以类别词开头、单条不超过 80 字、
以及**条目里不许出现括号**。为什么必须机器查：用户说的是"以后按这个写" ——
而这个仓库已经吃过一次同类亏（更新说明只推了标题那一行，而当时三条关于 notes 的断言**全都成立**，
判据太弱等于没有）。写这个检查时我自己也踩了两个坑（记在脚本注释里）：
条目里类别词后面**没有空格**（第一版按 `title + ' '` 判，全红）；以及第一版把整份文件当成
"一个版本"去判顺序（版本之间重复的段落被报成乱序）。

#### 74.2　发布 rc.4

内容 = rc.3 之后用户报的四件事 + 两次点头的两件事（详见 `CHANGELOG.md` 的 rc.4 一节，
那也是更新端点上 `notes` 的来源）：

① 实例目录被定位到 C 盘（用户报）→ 实例目录回到游戏根目录；
② 「还是其他盘的目录」→ 查出来是**修复版没部署到他机器上**（两份启动器都是旧构建）；
③ 「为什么还会显示 C 盘那个目录的游戏」→ 用户点头「ABC 全做」：删掉 C 盘残留 1.11 GB
   + 启动器自己的家搬到 `D:\IEML-launcher`；
④ 「切换页面不回最顶端」→ 换屏时滚回 0；
⑤ 更新日志格式定稿（本节 74.1）；
⑥ 更新按钮 / 更新日志页假承诺 / 启动时静默检查 + 有更新弹窗（rc.3 之后那批）。

产物与验证：

```
版本        : 0.1.0-rc.4（6 个文件 7 处一致；README 与 CHANGELOG 口径逐字相同）
安装包      : IEML_0.1.0-rc.4_x64-setup.exe  3.40 MB + .sig 420 字节（Ed25519）
桌面 exe    : 9,052,672 字节
sha256      : 2DE8104F48770B4E57AFE75630B958A3ABF33D413D7852C331072FEF3231FF1E
更新端点    : HTTP 200 version=0.1.0-rc.4；匿名拉包 3.40 MB；内置公钥验签通过（keyId 8B66ACD053761632）
notes       : 6715 字（CHANGELOG 的 rc.4 一节）
自检        : verify-manifest 18 项全绿 / check-exe-wiring 7 项全绿
门禁        : node tools/verify.mjs **25 项全过**（含发布版 exe 内嵌前端一致性）
真机探针    : 更新日志页渲染 4/4（五段有序、每条带类别词、最新一版是 rc.4、截图留存）
```

部署：桌面 `IEML.exe` 与快捷方式指向的安装版 `%LOCALAPPDATA%\IEML\ieml.exe` 都换成 rc.4
（安装版逐个核对了 sha256 与版本资源）。

★ 如实记：C 盘那 1.11 GB 是**用户点头之后在他这台机器上手动删的**，应用本身只做复制、不删源；
所以这一条**没有**写进用户向的更新日志 —— 别的机器上那份旧数据还在，写进去就是假承诺。

### 七十五、清理与合并：探针公共库、CSS 死规则、构建废品（2026-09-24）

用户一句「**代码清理 优化 合并，清理构建废品**」。这一节记**量出来的数**与**踩到的坑**，
以及"我**没有**动什么、为什么"。

#### 75.1　构建废品：15.7 GB → 只留交付物

| 位置 | 清之前 | 处置 |
|---|---|---|
| `src-tauri/target/debug` | **10.85 GB** / 14438 文件 | 整目录删（调试构建产物，随时可重建） |
| `src-tauri/target/release/{deps,build}` | **4.83 GB** | 删（中间产物；`release/ieml.exe` 与 `bundle/` 留着） |
| `bundle/nsis/IEML_0.1.0-rc.{1,2,3}*` | ~20 MB | 删旧安装包（线上发布仓里都有；本地留当前 rc.4） |
| `tmp/` 里的 6 份 `ieml-rc3*.exe.bak` | **51.6 MB** | 删（回退靠 git 里的那一版重新构建，不靠这些 exe） |
| `tmp/` 里的构建日志、旧截图、测试用 html | ~15 MB | 删（只留 `latest.json`、rc.4 的验收截图与校验产物） |
| `%TEMP%\ieml-*` 沙盒与单测临时目录 | 数百个目录 | 删 |

★ 删之前先量、删之后复量；`dist/`（0.8 MB，门禁要用）与 `node_modules`（82 MB）**没动**。
★ 顺序上**先跑门禁再删** —— 门禁里的 `cargo test` 会重建 `target/debug`，
反过来做等于白删（这一条写在这里，免得下次又按错的顺序做一遍）。

#### 75.2　合并：82 个探针里的重复胶水 → `tools/live/lib/cdp.mjs`

`tools/live/` 下 82 个用 CDP 的脚本、合计 17139 行，**每一个**都自带同样一段
60–80 行的管道（杀进程 → 起 exe → 轮询 `/json/list` → 建 WebSocket → 包 `Runtime.evaluate`
→ 收尾）。复制品的代价这个仓库已经付过一次：**两个阶段共用一个端口**那个假红
（旧进程被单实例挡掉，探针连上的是旧进程，于是"新构建"那一栏量到的是旧构建）。

抽出的 `lib/cdp.mjs` 只做管道（判据仍在各自探针里）：`launch / killIeml / waitNoIeml /
invokeOn / clickNav / ps / sleep` + 几个文件小工具（`mtimeOf / newestIn / countIn / missingIn / readOr / jsonOr`）。
本轮把自己这几轮的 6 个探针全部改过来（`probe-instance-root`、`probe-manifest-location`、
`probe-versions-page-dir`、`probe-own-root-move`、`probe-page-scroll-top`、`probe-changelog-format`），
并**逐个重跑确认判据没变**（4 个单阶段 + 2 个双进程）。历史探针**没有**动 —— 它们是过去的验收证据，
改它们只增加风险、不增加证据。

★ 迁移时踩到三件事，都写进了库里：

1. **端口用久了会脏**：探针各自写死端口（9971、9973…），一天跑几十次之后端口上全是
   TIME_WAIT，WebView2 再绑同一个端口就可能失败 —— 表现是"连不上 CDP"，
   甚至启动器**直接退出**（`exitCode=0`，日志里只剩一行 glass 设置）。
   改成 `pickFreePort()` 每次挑一个空闲端口。
2. **强杀会留下 WebView2 宿主进程**：一次实测残留 **10 个** `msedgewebview2.exe`，
   它们占着探针的 WebView 用户目录，把下一阶段的启动拖住。
   `killIeml()` 现在按"命令行里带我们探针 profile 名（`ieml-`）"把它们一起收掉 ——
   只按进程名一刀切会把用户别的高 WebView2 程序也杀掉。
3. **同一个 Node 进程里"起 → 杀 → 再起"不可靠**：第二阶段会直接退出（同 1 的表现），
   而把第二阶段**单独跑一个进程**完全正常（实测：`--phase=new` 一次就起来）。
   两个 A/B 探针因此改成**每个阶段各起一个进程**，结果写进 `tmp/*.json` 再比对。
   一开始以为是"单实例的锁没释放"，量过才知道不是 —— 同进程连跑这件事本身就是坑。

#### 75.3　代码里的重复：`should_adopt` 与 `is_newer`

`should_adopt`（账本该不该被源覆盖）里**又写了一遍** mtime 比较，与 `is_newer` 是同一个判据的
两份实现 ⇒ 合并成一处（"谁更新"只允许有一个来源，否则迟早两处给出不同答案）。
判据本身没变，仍由既有测试钉着（`adopt_records_lets_the_newer_copy_win_and_backs_up_the_old_one`、
`adopt_records_leaves_a_newer_target_alone`）。

#### 75.4　CSS 死规则：29 条（并且**第一次做错、回滚重做**）

用脚本按"类名在 `src/**/*.{ts,tsx,html}` 里连**词干**都找不到"筛出死类名，删掉对应规则：
**app.css 20 条 / pages.css 7 条 / tokens.css 2 条**（`.list-*` 那一族、`.acct-id-*`、
`.cw-*`、`.vi-top`、`.sr-only`、`.tnum` 等）。

★★ **第一版把活规则删了**（`.side-link`、`svg {}`、`@keyframes ieml-card-in`）：
它把**注释里**出现的 `.名字` 也当成选择器 —— 而这个仓库的注释里会引用类名
（甚至引用被删掉的旧类名）。修法四条，都写进了脚本：

① 先把注释从"选择器文本"里剥掉，只在真选择器上认类名；
② 选择器里的类名**全部**是死类名才删（组合选择器里有一个活的 → 整条留着）；
③ 没有类名的规则（`svg {}`、`@keyframes`、`*{}`）一律不碰；
④ 贴着规则的注释只在它**提到这个选择器**时才一起删，否则注释留下。

回滚是用 `git checkout -- src/styles` 做的（状态干净、一步还原），重做后**逐个核对被删的选择器**
（全是死选择器），再跑 `vite build` + 截图目视核对（启动/版本列表/设置/关于四页）。
...
### 七十七、降档必须**派生**，不能写盘（低性能损耗模式改掉用户档位）（2026-09-25）

【用户：「先把这个内存查了」→ 查的过程中发现：无论怎么把视效设成「灵动」，
重启之后 `prefs.json` 里都是 `mid`】

#### 77.1　根因：把"派生值"当成"用户选择"写回了盘

`SettingsPage` 的低性能损耗开关，打开时做了三件事：

```ts
lowPerfPrev.current = { motion, vfx: vfx.want };  // 原值 → 只存在内存
setMotion('lite');        // 写盘（localStorage）
vfx.choose('weak');       // 写盘（prefs.json）
```

于是：

* **用户的选择被覆盖** —— 盘上从「灵动 / 灵韵」变成「弱化 / 减少」；
* **同一次会话里关掉还能还原**（靠那个 ref）；
* **重启之后 ref 是 null** ⇒ 关掉开关什么都不还，档位永远停在降档值。

这与那个开关自己的承诺相反（第五轮用户要求的是「关掉时**还回用户原来的选择**」）。
`AppContext` 里其实早就有"派生"的写法（`vfxDowngradedByLowPerf`：低性能模式压过视效档），
只是当时只做了一半 —— 另一半由设置页"写盘 + 降两档"完成，删掉写盘之后那处不一致就露出来了
（它原来只把 aura 压到 `mid`，而 `choose('weak')` 是压到 `weak` 的；用户原话是「弱化视效」，
所以现在统一压到 `weak`）。

#### 77.2　修法：开关只翻自己，降档一律派生

* 开关：只改 `<html class="low-perf">` 与 `localStorage.ieml.lowPerf`；
* 视效：`vfxLevel = lowPerf ? 'weak' : decideVfx(want, cap).level`；
* 动效：`applyMotion(lowPerf ? 'lite' : readMotion())`；
* 用户的选择（`prefs.vfx` / `localStorage.ieml.motion`）**一个字节都不动**；
* 删掉 `lowPerfPrev` 那个 ref（它存在的唯一理由就是"记住被覆盖掉的原值"）。

同一屏内的一致性也补了一处：低性能模式开着时用户去点动效档位，现在会立刻把生效值压回
「减少」，并在提示里说清"已记住你的选择，低性能模式开着时实际跑减少"——
而不是显示一套、实际跑另一套。

#### 77.3　判据（`tools/live/probe-lowperf-keeps-user-level.mjs`）

★ 这条判据需要 `localStorage`（本机偏好就存在那里）跨进程活下来，
所以给探针库加了 `keepProfile`（默认仍清空配置目录）。

```
坏构建（rc.4）：
  ② 开低性能模式：生效 weak/lite，**盘上也变成 weak**   ✗
  ④ 重启后"用户选的"已是 weak/lite；关掉 → 仍是 weak/lite ✗（回不来）
好构建：
  ② 生效 weak/lite，**盘上仍是 aura/aura** ✓
  ④ 重启后"用户选的"仍是 aura/aura；关掉 → aura/aura ✓
```
...
| 项 | 实测 |
|---|---|
| 页面 JS 堆 | **3 MB**（DOM 110–180 节点） |
| 我们的数据（3 实例 + 缓存）带来的差 | ~14 MB |
| 一个实例 13 个进程合计 | 工作集 ~560 MB（私有 ~520 MB）：GPU 119–130 / 渲染器 ×2 112–122 / 其余是浏览器与工具进程 |

* 视效档 aura ↔ weak：556 ↔ 557 MB（**没差** —— "镜头/GL"在 aura 与 mid 下都启用）；
* 低性能损耗模式：内存 562 → 563 MB（没差），**空闲 CPU 35% → 1%**（它省的是 CPU，不是内存）。

**结论：没有可省的空间。** 那 560 MB 是 Chromium/WebView2 的结构 —— 不是我们的代码占的
（页面只有 3 MB 堆），也没有为了"看着小"去做无意义的改造。
这一条写下来是有意的：以后有人问"为什么一个启动器占 560 MB"，
答案在这里，而不是靠猜。

### 七十八、更新日志**不写归属**（"你报的"这类话一律不写）（2026-09-25）

用户（截图：应用内「更新日志」页的 rc.5 一节）：
...
```
两件事都是你报的，第二件是查第一件时顺手挖出来的。      ← 头条
（rc.4）这一版修好了三件你报上来的问题，…
（rc.3）这一版只改了你点出来的四处界面上的事，…
（rc.2）…修复了你自己点「取消」却弹红色「安装失败：任务被取消」。
```
...
| 版本 | 改之前 | 改之后 |
|---|---|---|
| rc.5 头条 | 两件事都是你报的，第二件是查第一件时顺手挖出来的。 | 这一版修掉了两件会影响使用的事，另外把低性能损耗模式的行为改正了。 |
| rc.4 头条 | 这一版修好了三件你报上来的问题，… | 这一版修好了三个问题，… |
| rc.3 头条 | 这一版只改了你点出来的四处界面上的事，没有新功能。 | 这一版只改了四处界面上的事，没有新功能。 |
| rc.2 条目 | 修复了**你自己**点「取消」却弹红色「安装失败：任务被取消」。 | 修复了点「取消」却弹红色「安装失败：任务被取消」。 |

* 这条规矩写进 `src/data/release-notes.ts` 文件头，作为模板的**第 ④ 条**（与"不吹自己"同一个方向：
  读者关心的是变化，不是功劳簿）；
* **进门禁**：`tools/check-release-notes.mjs` 新增第 ⑦ 条判据 —— 正文（剥掉注释之后的字符串字面量）
  里不许出现 `你报 / 我报 / 报上来 / 用户报 / 你点出来 / 用户反馈 / 用户指出`。
  先喂坏样本证明能红（头条里写「这一版修好了你报上来的三个问题」→ 两条报错如期出现）✓

★ 注意"你选的档位 / 你的版本目录"这类**不是归属**：它们说的是用户的设置与数据，
不是"这是谁发现的"，所以留着（改完那几处仍然自然可读）。

#### 78.2　关于已经发出去的 rc.5

* 桌面与安装版**重新构建后重新部署**（同一版本号 rc.5，只改了文案）；
* 更新仓里的 rc.5 产物也**重新上传**了一次 —— 这样"从 rc.3 / rc.4 更新上来"的机器拿到的是改过的文案；
* ★ 如实记：**已经装上 rc.5 的机器不会自动再装一次**（版本号没变，更新器认为已是最新）——
  它们的更新日志页要等下一版才会变成新文案。这一条不打算用"偷偷改版本号"的办法绕过去。

### 七十九、换游戏根目录**即时生效**：`AppPaths` 从"启动时定死"改成"可以当场换掉"（2026-09-25）
...
于是 `set_data_root` 只能做两件事：校验 + 把选择写进 `datadir.txt`，然后告诉用户"重启后生效"。
界面上那句 `★ 换完重启才生效` 就是这么来的 —— 它不是文案保守，是**事实如此**。

#### 79.2　改法（后端换句柄 + 前端刷派生数据）

* `AppState.paths`：`AppPaths` → **`RwLock<Arc<AppPaths>>`**，配三个口子：
  `paths()`（**克隆 Arc 出来用，不跨 await 持锁**）、`set_paths()`、`AppState::new(paths)`；
* `set_data_root`：校验 → `remember_root` → `AppPaths::from_root(target)` → `ensure()` / `ensure_own()`
  → **当场 `set_paths`** → 返回 `restart_required: false`。旧根**一个字节都不动**（语义仍是"只换地方，不搬东西"）；
* 前端 `reloadAfterRootChange()`（`AppContext`）：重读 `machine_info` + `load_instances`
  （`machine/set` 新 reducer），再 `rescanJava`；弹窗成功后派一条 toast；
* 界面上"重启后生效"那套**整块删掉**：`pendingRoot` 状态、设置页那个角标、它的 CSS
  （`.droot-pending`），以及弹窗底那行 `换完重启才生效` → `换完立刻生效，不用重启`。
...
| 探针 | 量什么 | 老构建（rc.5） | 新构建 |
|---|---|---|---|
| `tools/live/probe-live-root-switch.mjs` | 后端：同一进程里 `machine_info.data_dir` 与实例健康 | ①✅ ②✗ ③✗ ④✅ ⑤✅（空绿） | ①–⑤ 全 ✅ |
| `tools/live/probe-live-root-switch-ui.mjs` | 界面：设置页那行 / 弹窗文案 / 版本列表警告 | ②✗ ③✗ ④✗ | ①–⑤ 全 ✅ |

* 后端 ③ 是"句柄真换了"的硬证据：新根下没有 `.minecraft/versions`，
  于是 `instance_health` 立刻把 **3/3** 个实例标成"版本缺失"（老根下 0/3）；
* 界面 ③④ 是"用户看得见的那一半"：点「用这个」后 toast 说"即时生效"、弹窗自动关、
  设置页那行**当场**变新目录、切到版本列表就看到「有版本的磁盘文件已经不在了 · 3 个」。
  ★ 判据**不是**"实例没了"—— 账本住在启动器自己的家里（ADR-七十二），换根目录不动它；
  那 3 个条目还在，只是目录换成新根下的。**第一版判据就是这么写错的**（红了一次才发现）；
* "没刷页面"怎么证：换目录前后往 `window` 上钉一个标记，事后还在 = 同一个文档。

#### 79.4　几件想清楚才动手的事

* **正在跑的游戏不受影响**：`RunningGame` 存的是 `log_path`（绝对路径）与子进程句柄，
  `stop_game` / `running_games` 都不查根目录 —— 换目录不会让"停止"失灵；
  而"这一局的时长写回实例"那条回写 2026-09-24 就不在了，所以也不会把时长写进新根；
* **记账文件仍然住在 `%APPDATA%\IEML`**：它是**开机引导记录**（还不知道根目录时就读它），
  不能放进游戏根目录，否则换根 = 把自己搬走；
* 换目录**不重启、也不刷页面**：刷页面会把当前页 / 滚动位置 / 弹窗全丢掉，
  而这次切换只是一次数据源替换。

#### 79.5　记账文件是**全局状态**：探针开跑前摆正，跑完擦干净

红证跑完留下的现场：老构建把"换回来"当成「这就是当前的数据目录，没有变化」而**拒绝落盘**
（它以为当前根还是 `D:\IEML`，其实记录已经写成新目录了）⇒ 记账文件停在临时目录，
**下一次启动就真的按那个目录起来**。所以 `probe-live-root-switch*.mjs` 现在两头都擦
（`forceOldRecord`：开跑前 + 结束后各一次，并打印它擦了什么）。

★ 同一类事故在开发期真发生过一次：一次开发运行把记账文件指到了 `E:\IEML`（**仓库目录**），
应用忠实地照着它建了 `.minecraft` 与 `instances`（账本的实例被接管后抽了 61.7 MB natives 进去）。
没有报错、没有提示 —— **记录写错了，应用只会照做**。清理前逐个核对过：那 63 个文件在
`D:\IEML` 里都有同名同大小的原件，账本本身（`D:\IEML\instances.json`）里没有一个 `E:` 引用。

### 八十、版本列表必须**按当前游戏目录说话**（"点了没刷新"其实是没有一句话说清状态）（2026-09-25）
...
截图里那一页：**3 个条目全在报错**（「有版本的磁盘文件已经不在了 · 下面有 3 个版本…」+
「这些版本还没有游戏文件」），而用户刚在设置页把游戏根目录换走 —— 他看到的像"页面没刷新"。

★ 先说事实：那一页**已经刷新过了**（提示文字就是从那 3 个条目的当场体检来的），
真正的问题是**这一页没有一句话说清"你选的这个目录里什么都没有"**：

* 版本列表列的是**账本里的实例**，而账本住在启动器自己的家里（ADR-七十二），
  **与游戏根目录无关** ⇒ 换到空目录之后，条目一个都不会少，只是全部缺文件；
* 结果就是"换目录前"和"换目录后"看到的是同一张列表 + 一堆红字，谁也看不出目录换了。

#### 80.2　改法（三条，都在用户指的那条路上）

1. **后端给一个磁盘事实**：`machine_info` 新增 `version_count` = `<root>/.minecraft/versions`
   下的目录数（读不到 = 0，不编数）。`reloadAfterRootChange()` 本来就会重读 `machine_info`，
   所以换目录的那一刻这个数就跟着变了；
2. **进这一页就重读**：`VersionsPage` 挂载时 + `currentRoot` 一变，都调
   `refreshInstances()`（与"窗口重获焦点时刷新"**同一个函数**，2026-09-23 建的，不新开第二份），
   那条"版本文件还在不在"的体检也把 `currentRoot` 加进依赖 —— 一句话：**这一页的数据源只有一个**；
3. **把话说清楚**（在最上面，排在"文件不在了"之前）：
   「当前游戏目录里还没有版本 —— 现在用的是 `<路径>`，里面一个版本文件都没有；
   下面的条目引用的版本在别的目录里。想换回去：设置 → 存储 →「新建/切换…」」+ 一个「去设置里换目录」按钮。

★ 为什么判据用**磁盘事实**（`version_count === 0`）而不是"实例是不是都缺文件"：
后者分不清"刚换到空目录"与"用户自己在资源管理器里把版本删了" —— 而这两种要说不同的话
（后者已经有专门那条 Note，且 2026-09-23 明确要求**不替用户删条目**）。

#### 80.3　真机 A/B（`probe-live-root-switch-ui.mjs` 新增第 ④ 条）

```
刚发布的 rc.6（没这个修复）：④ ✗（页面上没有那条提示）　⑤ ✗（machine_info 里没有 version_count）
修复后的构建：              ④ ✓（提示在，且里面写着新路径）　⑤ ✓（版本数 22 > 0，回到 D:\IEML）
```

★ 顺手记一条现场教训：用户这次把根目录指到了 `E:\IEML` —— 那正是**本仓库的目录**，
因为「换一个游戏根目录」弹窗里按盘符给的候选就是 `<盘符>\IEML`，而 E 盘上恰好就是它。
不是 bug，但很容易看岔：以后看到"仓库里冒出 `.minecraft`/`instances`"，
先查 `%APPDATA%\IEML\datadir.txt`，别先怀疑代码。

### 八十一、版本列表改成**读"当前文件夹"**（PCL 的文件夹逻辑）（2026-09-25）

用户（两张 PCL2 截图，「版本选择」那张的左边是文件夹列表、右边是分组后的版本）：

> 你看PCL，就是像换了个文件夹去读游戏版本，可以无缝切换

#### 81.1　上一轮我理解错在哪

rc.7 我改的是**呈现**（把缺文件的折进「错误的版本」）—— 那是 PCL 列表的*形状*，
不是它的*数据来源*。PCL 的关键在这一句：**列表就是"这个文件夹里的版本"**，
换文件夹 = 换一份游戏数据。而 IEML 列的是**账本里的实例** ——
账本住在启动器自己的家里（ADR-七十二），与文件夹无关，
所以用户"换到 E 盘却还看见 D 盘那几条"，看着就像没刷新。

#### 81.2　改法（数据源 + 一条导入路径）

1. **后端 `folder_versions`**（`commands_real::scan_folder_versions`）：
   真读盘扫 `<root>/.minecraft/versions/*`，每个目录给出 `dir / id / inherits /
   mc_version / loader_name / has_json` —— **不看账本、不看网络清单**。
   加载器只按**目录名**认（`fabric-loader-…` / `…-forge-…` / `neoforge` / `quilt` / …）：
   要精确到"哪个加载器的哪个版本"得再读一遍 JSON，而列表行本来就会显示盘上事实，不值得多做一次读盘。
2. **「版本列表」的数据源换成它**（`VersionsPage`）：
   · 文件夹里的版本 = 列表里的行（≤ 有 JSON 的）；
   · 账本里的实例只负责给某个版本补上"你起的名字 / 设置 / 存档目录"——
     按「MC 版本相同 + 是不是加载器版本也相同」一对一认领；
   · **认领不到的既不删、也不显示**（用户看了折叠组那行之后：「既然不在这个文件夹就不用显示了」）。
     这一页的语义就是"这个文件夹里有什么"，不属于它的东西不占位置；账本**一条都没删** ——
     把目录换回去，那些实例立刻又出现在列表里（存档、Mod、设置都还在原地）；
   · 文件夹里有、账本里没有的版本 → 显示成「还没建实例」，点「用它建一个」就地建实例；
   · 页头常驻「N 个版本 · 文件夹 <路径>」（PCL 左侧那份"当前文件夹"）。
   ★ 顺带把 2026-09-23 那条"哪些实例的版本文件不在磁盘上"的体检也从这一页去掉了：
   "在不在"现在由**文件夹扫描**直接回答，再单独查一遍就是第二份判据（迟早说出不一样的话）。
   `instanceHealth` 命令本身还留着（启动页/探针在用）。
3. **认 `.minecraft` 本身**（PCL 的「添加已有文件夹」就是这么选的）：
   `normalize_root_target` —— 用户从资源管理器直接选到 `.minecraft` 时**自动取上一级**
   （否则会在它下面再建一层 `.minecraft`，版本永远是空的），并在返回里带
   `normalized_from`，界面照实说"你选的是 .minecraft，已按上级目录当根目录"。

#### 81.3　真机 A/B（`probe-versions-read-folder.mjs`，五条判据，全绿）

造一个"PCL 文件夹"，里面三份版本，其中 `1.21.4` **账本里没有**：

```
① 换过去后列表 = ["Minecraft 26.2", "Minecraft 26.2 + Fabric 0.19.5", "1.21.4"]
   （前两行是账本实例认领的结果，第三行只存在于这个文件夹里 ⇒ 证明列表来自文件夹）
...
★ 这一轮踩的**同一个坑踩了两次**，值得写下来：
`folder_versions` 的返回是 **snake_case**（`has_json`），而前端类型承诺 camelCase（`hasJson`）——
桥接层偷懒直接透传 ⇒ `v.hasJson` 是 `undefined` ⇒ 每份版本都被当成"没有 JSON" ⇒
**后端明明返回 3 个版本，界面上 0 行**。第二处是探针里读 `normalized_from` 时写成了 `normalizedFrom`。
教训：**跨 IPC 的字段名必须显式映射**（同文件里 `machineInfo` 一直是这么写的，就我这里偷懒了），
