...
> ★★ **2026-09-26 加注：这张表里有三行与实测不符 —— 照实标出来，别按它去验收。**
>
> | 维度 | 表里写的 | 实测 / 实际口径（证据） |
> |---|---|---|
> | 空闲内存占用 | ≤ 60 MB | ❌ **整棵进程树约 560 MB**（一个窗口 = 13 个进程，其中 Rust 主进程 30.5 MB，其余是 WebView2/Chromium 的结构性开销）。见 `docs/DECISIONS.md` 第 8069 行一带与 `CHANGELOG.md` 的 rc.5 那节：「**没有可省的空间**」。★ 这一行**没有被正式撤销过**，所以留着当"目标"看是不诚实的 —— 实测值就是上面这个 |
> | 支持平台 | Windows 10+ / macOS 11+ / Linux | ⚠️ **0.1.0 只发 Windows**（明文口径见 `docs/VERSIONING.md` 的「平台口径」：仓库没有 CI、macOS/Linux 一台机器都没跑过，而 Rust 侧确有 Windows 专属代码）。跨平台推到 `0.2.0` |
...
> 不抓 HTML 表格）。
>
> ★ 保留原表不改写：它记的是**当初定下的目标**；这一块加注记的是**今天的事实**。
...
**并发下载调度**（这是启动器性能的核心）：
- 用 `tokio` + `futures::stream::buffer_unordered`，并发度默认 **64**
  （`net/source.rs` 的 `DEFAULT_CONCURRENCY`），下限 **4**（`MIN_CONCURRENCY`）。
- ★ **并发度是按源自己调的，不是一个固定常数**（2026-09-17 更正本节原先写的
  "默认 16（可配 8–64）" —— 三个数字都对不上代码）：某个源返回 429（限流）就把
  提示值**减半**（不低于 4），之后每成功一轮**回升 25%**（不超过 64）。
  实际并发取 `min(调用方想要的值, 当前提示值)`。
  固定并发度做不到这一点：调小了慢，调大了会被限流。
...
| **CurseForge** | 全部走国内镜像（`mod.mcimirror.top`），**不需要任何 key**，界面也不提供填写入口 |
...
5. **依赖审计**：CI 里跑 `cargo audit` + `npm audit`，锁定依赖版本 ★ 2026-09-26 更正：**没有 CI**，这两条审计**目前没有跑**（依赖版本由 `pnpm-lock.yaml` / `Cargo.lock` 锁住）
...
> ★★ 2026-09-26：这里原来还画着一行 `.github/workflows/  # CI: 三平台构建 + 发布` ——
> **那个目录不存在**（全仓库没有任何 workflow 文件）。交付门禁 `node tools/verify.mjs`
> 是在本机手动跑的；CI 与三平台构建仍是 `0.2.0` 的事（见第 1 节那段加注）。
> ⇒ 画在"目录结构"里的一行不存在的东西，比写在文档正文里更容易骗到人（会照着它去找）。
...
| OptiFine 无官方 API | 中 | 解析版本清单页 + 本地缓存 + 允许手动指定版本号兜底 ★ 2026-09-26 更正：**实际走 BMCLAPI 的结构化 JSON**（`net/optifine.rs`），不抓 HTML 表格；版本号由在线清单给 |
| LiteLoader 等老加载器停止维护 | 中 | 保留支持但不投入优化；标记为"已停止维护" |
| 跨平台 WebView 渲染差异 | 中 | CI 三平台视觉回归 + 锁定 CSS 特性集 ★ 2026-09-26 更正：**没有 CI**；0.1.0 只发 Windows，所以这条暂时不适用（推跨平台时要先补 CI 与一台真机） |
| 大整合包下载体积大、耗时长 | 中 | 高并发 + 断点续传 + 进度可暂停恢复 + 文件去重 |
| Mac 公证流程繁琐 | 中 | 提前配好 GitHub Actions 签名流水线 ★ 2026-09-26 更正：**没有 CI、也没有 macOS 版**；见 `docs/VERSIONING.md` 的平台口径 |
